...
class HarvestCommonsEnv(MapEnv):
    ...
    def step(self, actions):
        nObservations, nRewards, nDone, nInfo = super().step(actions)
    ...
    def get_social_metrics(self):
        metrics = tuple([np.mean(self.metrics[metric]) for metric in self.metrics.keys()])
        self.metrics = {"efficiency": [],
                        "equality": [],
                        "sustainability": [],
                        "peace": []}
