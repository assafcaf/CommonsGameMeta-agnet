...
from Danfoa.envs.utils.utility_funcs import rgb2gray
...
class MapEnv(MultiAgentEnv):
    def __init__(self, ascii_map, num_agents=1,  color_map=None, ep_length=1000, view_len=10):
        """
        ...
        self.setup_agents()
        self.return_agent_actions = False
        self.t = 0
        super().__init__()

    @property
    def observation_space(self):
        # TODO change observations shape
        obs_space = {
            "curr_obs": Box(
                low=0,
                high=255,
                shape=(2 * self.view_len + 1, 2 * self.view_len + 1, 1),
                dtype=np.uint8)}
    ...
    def render(self, filename=None, title=None, mode="RGB"):
    ...
    def update_map_fire(self, firing_pos, firing_orientation, fire_len, fire_char, cell_types=[],
        ...
        # width of 5
        firing_pos = [start_pos + right_shift * 2 - firing_direction, start_pos + right_shift - firing_direction,
                      start_pos,
                      start_pos - right_shift - firing_direction, start_pos - right_shift * 2 - firing_direction]
